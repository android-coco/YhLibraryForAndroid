...
public class ItemViewDelegateManager<D>
{
    ...
    public int getItemViewType(D item, int position)
    {
        ...
        throw new IllegalArgumentException(
                "No I_ItemViewDelegate added that matches position=" + position + " in data source");
    }
    ...
    public I_ItemViewDelegate getItemViewDelegate(D item, int position)
    {
        int delegatesCount = delegates.size();
        for (int i = delegatesCount - 1; i >= 0; i--)
        {
    ...
    public int getItemViewLayoutId(D item, int position)
    {
